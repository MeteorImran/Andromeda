...
def _bootstrap() -> None:
    ...
    if not need:
        return
    print(f"\n[ANDROMEDA] First-run setup — installing: {', '.join(need)}")
    print("[ANDROMEDA] This happens only once.\n")
    _sp.run([_sys.executable, "-m", "pip", "install", *need], check=True)
    print("\n[ANDROMEDA] Base packages ready — restarting…\n")
...
# We import JarvisUI but alias it to AndromedaUI to match your ui.py class name
from ui import JarvisUI as AndromedaUI
...
TOOL_DECLARATIONS = [
    ...
    {
        "name": "send_message",
        "description": "Sends a message to a specific person via WhatsApp, Telegram, or similar.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    },
    {
        ...
        "description": "Controls the computer: volume, brightness, window management, keyboard shortcuts, typing text on screen, closing apps, fullscreen, dark mode, WiFi, restart, shutdown, scrolling, tab management, zoom, screenshots, lock screen, refresh/reload page.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    },
    {
        ...
        "description": "Controls any web browser. Use for: opening websites, searching the web, clicking elements, filling forms, scrolling, screenshots, navigation.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    {
        "name": "agent_task",
        "description": "Executes complex multi-step tasks requiring multiple different tools.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    {
        "name": "game_updater",
        "description": "THE ONLY tool for ANY Steam or Epic Games request. Use for: installing, downloading, updating games, listing installed games.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    },
    {
        ...
        "description": "Shuts down the assistant completely. Call this when the user expresses intent to end the conversation.",
        "parameters": {"type": "OBJECT", "properties": {}}
    },
    {
        "name": "file_processor",
        "description": "Processes any file that the user has uploaded or dropped onto the interface.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    {
        "name": "save_memory",
        "description": "Save a personal fact about the user to permanent long-term memory. MANDATORY: call this IMMEDIATELY (without asking) whenever the user states or corrects a personal detail.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "category": {
                ...
                "key":   {"type": "STRING", "description": "Short snake_case key, e.g. 'name', 'age'"},
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Andromeda, an enterprise orchestration autopilot. "
...
# ---------------------------------------------------------------------------
# AndromedaLocal (Renamed logic class)
# ---------------------------------------------------------------------------

class AndromedaLocal:
    def __init__(self, ui: AndromedaUI):
        self.ui               = ui
    ...
    def _do_reconfigure(self, new_config: dict) -> None:
        ...
        try:
            ...
            self._tts_ready.set()
            self.ui.write_log("SYS: Voice Synthesis reconfigured.")
        except Exception as e:
        ...
        if old_stt_engine == new_stt_engine:
            try:
                ...
                self.ui.write_log("SYS: Acoustic Sensor reconfigured.")
            except Exception as e:
                self.ui.write_log(f"ERR: STT reconfigure — {e}")
        else:
            self.ui.write_log("SYS: Acoustic engine changed — restart required.")

        if new_config.get("llm_model", "") != old_llm_model:
            self.ui.write_log("SYS: Warming up Cognitive Engine…")
            from core.llm_client import warmup_model
            warmup_model()
            self.ui.write_log("SYS: Cognitive Engine ready.")
    ...
    def _execute_tool(self, name: str, args: dict) -> str:
        ...
        try:
            if name == "open_app":
                ...
                def _shutdown():
                    import time, os
                    self.speak("Andromeda power down sequence initiated. Goodbye.")
                    time.sleep(3.5)
                    os._exit(0)
        ...
        print(f"[ANDROMEDA] 📤 {name} → {str(result)[:80]}")
        return result
        ...
        for _round in range(MAX_TOOL_ROUNDS):
            ...
            if not final_tool_calls:
                if _streamed:
                    ...
                    self.ui.write_log(f"Andromeda: {final_content}")
                elif final_content:
                    assistant_msg = {"role": "assistant", "content": final_content}
                    messages.append(assistant_msg)
                    self._conversation.append(assistant_msg)
            ...
            if all_silent:
                ...
                self.ui.write_log(f"Andromeda: {_ack}")
                self.speak(_ack)
                break
            ...
            if _tool_results and not any(n in _NEEDS_LLM_ROUND for n, _ in _tool_results):
                ...
                messages.append(_amsg)
                self._conversation.append(_amsg)
                self.ui.write_log(f"Andromeda: {_reply}")
                self.speak(_reply)
                break
    ...
    def _listen_whisper(self) -> None:
        ...
        try:
            with sd.InputStream(
                ...
                self.ui.write_log("SYS: Acoustic Sensor active (Whisper STT).")
                while True:
                    try:
            ...
            print(f"[STT-Whisper] Sensor error: {e}")
            traceback.print_exc()
    ...
    def _listen_vosk(self) -> None:
        ...
        try:
            with sd.InputStream(
                ...
                self.ui.write_log("SYS: Acoustic Sensor active (Vosk STT).")
                while True:
                    try:
                        chunk = q.get(timeout=0.1)
            ...
            print(f"[STT-Vosk] Sensor error: {e}")
            traceback.print_exc()
    ...
    def run(self) -> None:
        try:
            ...
            self.ui.write_log("SYS: Checking Cognitive Engine…")
            if ensure_ollama_running():
                self.ui.write_log("SYS: Engine OK.")
            else:
                self.ui.write_log("ERR: Engine unavailable — run: ollama serve")
            ...
            def _do_warmup():
                try:
                    ...
                    self.ui.write_log(f"ERR: Engine warmup — {e}")
            ...
            def _do_stt():
                try:
                    ...
                    self.ui.write_log("SYS: Acoustic Sensor ready.")
            ...
            self.ui.write_log("SYS: Booting nodes in parallel…")
            ...
            self.ui.write_log("SYS: ANDROMEDA online.")
            self.ui.set_state("LISTENING")
            self.ui.set_startup_status("● Autopilot active · System loading nodes…")
...
def main() -> None:
    ...
    def runner():
        ...
        andromeda = AndromedaLocal(ui)
        try:
            andromeda.run()
        except KeyboardInterrupt:
            print("\n[ANDROMEDA] Shutting down…")

    threading.Thread(target=runner, daemon=True).start()
    ui.root.mainloop()

if __name__ == "__main__":
    print(">>> INITIATING ANDROMEDA BOOT SEQUENCE...")
    main()